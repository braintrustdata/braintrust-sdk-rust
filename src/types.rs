...
pub(crate) struct Logs3Row {
    ...
    /// Paths that should stop deep-merging at this level (replacement instead of merge).
    /// Only used when `is_merge` is true.
    #[serde(rename = "_merge_paths", skip_serializing_if = "Option::is_none")]
    pub merge_paths: Option<Vec<Vec<String>>>,
    pub span_id: String,
